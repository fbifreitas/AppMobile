...
- [2026-04-01] Ponto de restauração do ambiente local documentado em `docs/05-operations/runbooks/PONTO_RESTAURACAO_AMBIENTE_LOCAL.md` — consultar sempre que retomar setup de ambiente web em nova sessão.
- [2026-04-01] Ao transferir contexto para novo agente, usar a secao 9 (Regras de Trabalho Compartilhadas + Prompt pronto) em `docs/05-operations/runbooks/PONTO_RESTAURACAO_AMBIENTE_LOCAL.md` para preservar governanca, padrao de testes e gate de promocao para `main`.
- [2026-04-01] Para BOW-054, publicar o Canonical Domain v1 no proprio `docs/05-operations/tactical-backlogs/BACKLOG_BACKOFFICE_WEB.md` (glossario, transicoes, ACL e invariantes) evita dispersao em arquivos novos e acelera onboarding de novos agentes.
...
- [2026-04-03] Em PowerShell no Windows, o argumento Maven `-Dtest=Classe1,Classe2` pode ser interpretado incorretamente; usar aspas no parâmetro inteiro ou rodar cada classe explicitamente evita parser error antes do Surefire.
- [2026-04-03] Ao adicionar FK nova em testes de integração (`integration_demands -> jobs/cases`), revisar imediatamente a ordem de `deleteAll()` no setup/teardown; a limpeza deve começar pelas tabelas filhas para evitar `DataIntegrityViolationException` mascarando a validação funcional.
- [2026-04-03] No terminal compartilhado do VS Code em Windows, não executar múltiplos `mvn test` em paralelo; os comandos competem pelo mesmo shell e podem disparar o prompt espúrio `Deseja finalizar o arquivo em lotes`. Serializar a execução e, quando necessário, usar `-DforkCount=0` foi o caminho estável nesta sessão.
- [2026-04-03] Ao substituir um stub mobile por resolução real baseada em configuração já existente, preferir extrair um ponto de entrada reutilizável no serviço canônico (`resolveForMobile`) em vez de duplicar a lógica de selector/rollout no módulo mobile.
...
- [2026-04-03] Em promocao de PR para `main` com mantenedor unico, NAO usar merge administrativo direto (`--admin`) como caminho padrao. O fluxo correto e: (1) reduzir temporariamente `required_approving_review_count` para `0`, (2) fazer merge da PR pelo caminho normal, (3) restaurar imediatamente para `1`, (4) registrar evidencias no ponto de restauracao.
- [2026-04-03] Para evoluir contrato mobile sem quebra, adicionar campos novos no topo (`publishedAt`, `sections`) mantendo payload legado (`step1/step2`) permitiu entregar modelo canônico NBR incrementalmente sem bloquear clientes atuais.
- [2026-04-03] Ao introduzir agregado explícito com FKs (ex.: `inspections` -> `inspection_submissions`/`jobs`), atualizar imediatamente a ordem de cleanup dos testes para remover primeiro o agregado novo evita falhas de FK intermitentes na suíte.
...
- [2026-04-05] Causa raiz de regressao de release: o fix de permissao Android (`378903d`, 2026-04-05 09:32 -03) ficou somente em `codex/mvp-pack-2-20260405`/`release/v1.2.30+50` e nao foi promovido para `origin/main` (main estava em `f8566d5`). Um pacote novo (`codex/mvp-final-program-20260405`) foi iniciado a partir de `main` e seguiu para `release/v1.2.32+52` sem esse commit. Regra preventiva obrigatoria: antes de abrir release, executar gate de abrangencia `git log origin/main..HEAD --oneline` + checklist de "fixes criticos esperados" (IDs de commit/cards) e bloquear promocao se qualquer fix critico estiver fora do range da release.
- [2026-04-05] Incidente recorrente em promocao de PR: `gh api` retornou erro por tipagem quando boolean/integer foi enviado com `-f` (string). Regra obrigatoria: usar `-F` para `required_approving_review_count`, `dismiss_stale_reviews` e `require_code_owner_reviews`; se houver `HTTP 422`, abortar promocao, corrigir tipagem e repetir o PATCH antes de seguir.
- [2026-04-05] No mobile, fechamento de INT-003/BL-057 exige validar duas bordas em conjunto: (1) assinatura HMAC valida/invalida do payload remoto de `checkin-config` e (2) efeito da politica dinamica `visivel/obrigatoria` na UX do Check-in. Cobrir apenas o service nao basta; o widget do fluxo precisa provar ocultacao da CTA e bloqueio da camera quando a Etapa 2 for obrigatoria.
- [2026-04-05] Para sync real mobile (INT-006/INT-007), `payload.toString()` nao e contrato suficiente para idempotencia. A derivacao estavel precisa usar serializacao canonica do payload e combinar explicitamente `jobId + exportedAt + payloadChecksum`, senao a mesma vistoria pode gerar chaves diferentes por ordem de mapa.
- [2026-04-05] A reconciliacao de fila offline nao termina no `flush`: quando o backend devolve protocolo real para um envio pendente, o app precisa aplicar `idExterno/protocoloExterno` de volta ao job local por `jobId`, inclusive no bootstrap/manual refresh da Home, para manter rastreabilidade operacional apos a drenagem automatica.
- [2026-04-05] Ao sanear o core configuravel da vistoria, extrair primeiro servicos compartilhados de semantica (`semanticKey`, aliases, `labelsBySurface`) e de obrigatoriedade antes de tocar na UX operacional evita espalhar regra de negocio em `CheckinScreen`, `OverlayCameraScreen` e `InspectionReviewScreen`. O fallback deve continuar forte, mas consumindo o mesmo contrato semantico do remoto.
- [2026-04-05] Em retomada da vistoria, o recovery da revisao nao pode depender apenas do requisito pendente. Persistir `review.cameraContext` com o ultimo contexto real capturado (`macroLocal`, `ambiente`, `elemento`, `material`, `estado`, incluindo `ambienteBase`/`ambienteInstanceIndex`) evita reabrir a camera em contexto errado e preserva a operacao com ambientes repetidos.
- [2026-04-06] Na refatoracao V2 do fluxo configuravel, o primeiro corte estrutural mais seguro foi criar estado explicito de captura e um adapter de recovery sem reescrever a UI. Reduzir strings soltas dentro de `OverlayCameraScreen` e tirar a serializacao manual de `cameraContext` da revisao entregou valor arquitetural sem romper payload legado.
- [2026-04-06] Em suites Flutter que validam labels de revisao/configuracao, texto exibido pode degradar por encoding do arquivo/terminal sem afetar a regra funcional. Para labels sensiveis de accordion/status, preferir helper de teste com comparacao normalizada e estabilizar a UI com strings/escapes consistentes antes de concluir que a regressao e de negocio.
- [2026-04-06] Para quebrar um service concentrador sem congelar a entrega, a ordem mais segura foi: (1) extrair loader/merge de documento, (2) extrair store de preferencias (`usage/prediction`), (3) manter o service original como facade temporaria. Isso reduz responsabilidade real sem forcar rename amplo ou troca imediata de chamadas nos widgets.
- [2026-04-06] Para concluir a Onda 3 sem quebrar a navegacao, a ordem incremental que funcionou foi: request coeso do coordinator -> resolver de contexto inicial -> adapter de recovery -> acoes contextuais/taxonomia -> resolvedor de menu/view-state -> service de transicao da camera -> apresentacao semantica dos niveis -> catalogo/inteligencia do menu. Cada corte retirou regra real da UI antes de introduzir a proxima camada.
- [2026-04-06] Ao mover intelligence/catalogo para fora da fachada, preservar o formato persistido (`usage`/`prediction`) e operar por snapshots/mappers internos foi a estrategia mais segura para manter compatibilidade durante a transicao sem refazer payloads legados.
- [2026-04-06] No fechamento obrigatorio das fronteiras camera/revisao, tirar apenas o contrato publico legado nao basta. Sempre que a tela passa a renderizar a partir de estado derivado novo (ex.: secoes de seletores inline), o mesmo estado precisa ser recalculado tambem nos fluxos de teste/bootstrap e apos cada transicao local, senao `useTestMenuData` mascara regressao de renderizacao real.
- [2026-04-06] Quando a revisao ainda persiste payload auxiliar legado (`cameraContext`) por compatibilidade, a serializacao e a remontagem do request devem ficar no mesmo adapter de recovery. Separar leitura numa tela e escrita em outra reintroduz drift arquitetural e torna a compatibilidade de transicao dificil de provar por teste.
- [2026-04-06] Na ultima milha de um refactor arquitetural, vale uma rodada extra apenas para esvaziar tipos privados de dominio/apresentacao ainda dentro do widget. Mover `EditableCapture`/grupos/status da revisao para um modelo dedicado reduziu acoplamento real sem mexer em navegacao ou payload.
- [2026-04-06] Para reduzir risco antes da implantacao, o corte final mais seguro foi separar primeiro servicos de apresentacao/roteamento (voz, painel, atalhos, mensagens tecnicas) e so depois consolidar o commit. Isso baixou o residuo de regra critica na UI sem abrir outra frente de arquitetura.
