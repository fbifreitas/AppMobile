...
|---|---|---|---|---|---|
| 1 | INT-001 | API Gateway de Integracao | Critica | Em andamento (filtro de gateway mobile com validaÁ„o de vers„o e rate-limit base em 2026-04-04) | Gateway dedicado para canais mobile com politicas de auth, rate limit, idempotencia e auditoria |
| 2 | INT-002 | Contratos versionados Web-Mobile | Critica | Em andamento (header obrigatÛrio X-Api-Version v1 aplicado em 2026-04-04) | OpenAPI versionada + politica de compatibilidade retroativa + deprecation policy |
...
| 16 | INT-016 | Testes de contrato automatizados | Critica | Em andamento (suite de contrato mobile ajustada para v1 e headers obrigatÛrios em 2026-04-04) | CI bloqueia merge em caso de quebra de contrato mobile-web |
...
| 25 | INT-025 | Pol√≠tica de versionamento de APIs e eventos | Critica | Concluido (gate estrutural + semantico no CI, suite de 8 testes do validador, hardening de readiness/fallback de endpoint OpenAPI e tolerancia a baseline ausente no ciclo de promocao para main em 2026-04-02) | Regras formais de compatibilidade retroativa e deprecia√ß√£o com gate em CI para breaking changes |
| 26 | INT-026 | Context envelope obrigat√≥rio (tenant + correlation + actor) | Critica | Em andamento (config module enforcement + web client auto-propagation complete; RequestContextValidator utility reusable; pendente: expansion to INT-026 integration routers para inbound calls from external apis following same pattern) | Toda chamada s√≠ncrona e evento ass√≠ncrono deve carregar contexto m√≠nimo validado no gateway |
| 27 | INT-027 | Padr√£o de idempotency-key por opera√ß√£o | Critica | Pendente | Chaves idempotentes normalizadas por dom√≠nio, TTL definido e reprocessamento seguro sem duplicidade |
| 28 | INT-028 | Contrato de erro can√¥nico entre canais | Alta | Em andamento (funda√ß√£o v1 + cobertura TDD ampliada nos endpoints mobile cr√≠ticos) | Cat√°logo √∫nico de erros com c√≥digos, severidade e orienta√ß√£o operacional consistente para web/mobile |
...
- INT-001: filtro MobileGatewayPolicyFilter adicionado no backend para rotas /api/mobile/* com validaÁ„o de vers„o e rate-limit base por tenant/actor.
- INT-002: contratos mobile passaram a exigir X-Api-Version (1) em endpoint e validaÁ„o de contexto.
- INT-016: testes de contrato/integraÁ„o mobile atualizados para novo header obrigatÛrio e polÌtica de vers„o.
