# GEMINI.md — Repository-Specific Guidance (V2)

## Official direction

This repository must be treated as a corporate ecosystem platform:
...
Inspection remains strategic, but it is not the global platform identity.

## Classification rule

Every proposal must classify artifacts into one of these layers:
...
If classification is unclear, do not default to `domain-inspection`.

## Mandatory reading before significant changes
...
- Platform Core must stay domain-agnostic.
- Shared Foundations must remain neutral and reusable.
- Domain packs are independent.
- Cross-domain interactions must use contracts/APIs/events/read models/ACL.
- White-label is implemented by configuration/policy, never code fork.
- Tenant is structural for authorization, data isolation, integrations and observability.

## Naming rules

Do not promote inspection-specific terms to global artifacts.

Keep these under `domain-inspection` scope:
...
Prefer neutral terms for shared abstractions:
...
Do not collapse corporate and domain canonical models.

## Documentation coupling

If a change affects architecture, naming, backlog, tenant/white-label, domain boundary, shared API or module placement, update the corresponding docs in the same change.

## Legacy policy

Historical inspection-centric documents may exist, but must stay in `docs/legacy/` or be clearly marked as legacy.
