...
export function GET(request: NextRequest) {
  ...
  return callBackendOperationsApi("backoffice/intelligence/capture-gates", {
    headers: buildAuthenticatedHeaders(session, "capture-gates")
  }, undefined, {
    tenantId: session.tenantId,
    actorId: String(session.userId),
    correlationPrefix: "capture-gates"
  })
    .then(({ status, payload }) => NextResponse.json(payload, { status }))
    .catch(() =>
      NextResponse.json(
        { error: "Falha ao consultar capture gates no backend" },
        { status: 502 }
      )
    );
}
