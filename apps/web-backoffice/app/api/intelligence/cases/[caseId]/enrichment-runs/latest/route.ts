...
export function GET(
  ...
  return callBackendOperationsApi(`backoffice/intelligence/cases/${caseId}/enrichment-runs/latest`, {
    headers: buildAuthenticatedHeaders(session, "latest-enrichment-run")
  }, undefined, {
    tenantId: session.tenantId,
    actorId: String(session.userId),
    correlationPrefix: "latest-enrichment-run"
  })
    .then(({ status, payload }) => NextResponse.json(payload, { status }))
    .catch(() =>
      NextResponse.json(
        { error: "Falha ao consultar o ultimo enrichment run no backend" },
        { status: 502 }
      )
    );
}
